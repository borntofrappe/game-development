...
function love.load()
  love.window.setTitle('Detecting Collision')
...
function love.update(dt)
  ...
  if love.keyboard.isDown('left') then
    -- update the position of the character, conditional to ground tiles not being present on the left of the character
    topLeft = pointToTile(gCharacter.x - 1, gCharacter.y - 1)
    bottomLeft = pointToTile(gCharacter.x - 1, gCharacter.y + CHARACTER_HEIGHT - 1)

    -- move only if the tiles are sky tiles
    if topLeft.id == TILE_SKY and bottomLeft.id == TILE_SKY then
...
function love.draw()
  ...
  love.graphics.print('Detecting Collision', 8, 8)


  love.graphics.setFont(gFonts['small'])
  love.graphics.setColor(1, 1, 1, 1)
  love.graphics.print('Feel free to jump around', 8, 32)
  ...
  for y = 1, MAP_HEIGHT do
    ...
    for x = 1, MAP_WIDTH do
      table.insert(tiles[y], {
        -- include x and y describing the position of the tile in the table
        x = x,
        y = y,
        id = TILE_SKY,
        topper = false
...
-- function returning the tile identified by an x and y coordinate
function pointToTile(x, y)
  -- return nil if the x and y coordinate exceed the boundaries of the world
  if x < 0 or x > MAP_WIDTH * TILE_SIZE or y < 0 or y > MAP_HEIGHT * TILE_SIZE then
    return nil
  end

  -- return the tile identified by x and y cognizant of the size of each tile
  tile = tiles[math.floor(y / TILE_SIZE) + 1][math.floor(x / TILE_SIZE) + 1]
  return tile
end