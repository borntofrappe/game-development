...
function GenerateQuadsCreatures(atlas, width, height)
  local quads = {}
  local x = 0
  local y = 0

  local varieties = 3
  local rows = math.floor(atlas:getHeight() / height)
  local columns = 2
  ...
  for i = 1, rows do
    ...
    for j = 1, columns do
      quads[counter] = {}
      for k = 1, varieties do
        quads[counter][k] = love.graphics.newQuad(x, y, width, height, atlas:getDimensions())
        x = x + width
      end
      counter = counter + 1
    end
