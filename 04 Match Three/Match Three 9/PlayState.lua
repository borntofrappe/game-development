...
-- in the enter function set up the variables of the game, either received from the level state or initialized from the start state

-- in the init() function set up the variables for the game
    ...
    if matches then
      ...
      for k, match in pairs(matches) do
        ...
        for j, tile in pairs(match) do
          -- add 50 points per tile
          self.score = self.score + 50

          -- call the function to remove the matches
          self.board:removeMatches()

          -- check if the score surpasses the goal
          if self.score >= self.goal then
            -- set isPlaying to false, to avoid interacting with the board
            self.isPlaying = false
