...
function PlayState:update(dt)
  if not self.isTweening then
    ...
    if love.keyboard.waspressed("right") then
      self.board.selectedTile.x = math.min(COLUMNS, self.board.selectedTile.x + 1)
    end

    if love.keyboard.waspressed("left") then
      self.board.selectedTile.x = math.max(1, self.board.selectedTile.x - 1)
    end

    if love.keyboard.waspressed("up") then
      self.board.selectedTile.y = math.max(1, self.board.selectedTile.y - 1)
    end

    if love.keyboard.waspressed("down") then
      self.board.selectedTile.y = math.min(ROWS, self.board.selectedTile.y + 1)
    end
