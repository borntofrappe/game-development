...
function love.update(dt)
  ...
  if timerPipes > intervalPipes then
    ...
    -- clamp between 10 from the top and the smaller between [-30,30] around the previous coordinate and an arbitrary value from the bottom (equal to the height of the pipe and gap)
    local y = math.max(-PIPE_HEIGHT + 10, math.min(lastY + math.random(-30, 30), VIRTUAL_HEIGHT - GAP_HEIGHT - PIPE_HEIGHT))
  ...
  -- ! with a conditional check if the pair has a flag of true, and if so remove the pair from the table
